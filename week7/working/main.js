...
function getAddress (elem) {
	...
	if (text != null) {
		...
		text[0].split(',').slice(0,-1).forEach(function(d, i) {
			...
			line = line.replace(/[()\t\n]+/g, '').replace(/ +/g, " ").trim().replace(/-/g, "").trim().replace("-", ""); // get rid of cruft
			lines.push(line);
			});
			if(e == "58-66 West 135th St") {
				e = "58 West 135th St"
				}
	  // console.log(lines);
...
//for (var i = 1; i <= 10; i++) {
	...
	//if (i < 10)
		...
		content += fs.readFileSync(`data/m08.txt`);
	//else
...
fs.writeFileSync(`output/d8.json`, JSON.stringify(data));
